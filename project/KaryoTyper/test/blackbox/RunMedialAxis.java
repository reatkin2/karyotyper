...
public class RunMedialAxis extends JFrame {
		/**
		 * 
		 */
		...
		public RunMedialAxis(String string) {
			super(string);
			RunMedialAxis.closing = false;
		...
		public static void main(String[] args) {
			if (args.length < 1) {
				...
				// int imgCounter=0;
				RunMedialAxis frame = new RunMedialAxis("chromosome Getter GUI");
				...
				RunMedialAxis.currentStatus = new JLabel("Waiting for images");
				RunMedialAxis.currentStatus.setForeground(Color.RED);
				frame.add(upper);
				frame.add(lower);
				upper.add(RunMedialAxis.currentStatus);
				lower.add(imgCount);
				...
				while (!RunMedialAxis.closing) {
					...
					if (filename != null) {
						if (!RunMedialAxis.currentStatus.getText().contains("Finishing")) {
							RunMedialAxis.currentStatus
						...
						if (!RunMedialAxis.currentStatus.getText().contains("Finishing")) {
							RunMedialAxis.currentStatus.setText("Waiting for images");
						} else {
							RunMedialAxis.currentStatus.setText("Finished looking at img"
