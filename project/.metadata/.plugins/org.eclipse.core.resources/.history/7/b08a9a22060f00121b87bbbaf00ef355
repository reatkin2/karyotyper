...
public class RunChromosomeGetter extends JFrame{
				...
				//System.out.println(args[i]+"---------nextFilestarts Here---------------");
					...
					if(filename!=null){
						if(!RunChromosomeGetter.currentStatus.getText().contains("Finishing")){
							RunChromosomeGetter.currentStatus.setText("Finding Chromosomes in slide image: "+filename);
						}
						...
						if(!RunChromosomeGetter.currentStatus.getText().contains("Finishing")){
							...
							RunChromosomeGetter.currentStatus.setText("Finished looking at img"+filename+" shutting down.");
						}
					}
				try {
					Thread.sleep(10);
	...
	protected static void exitProcedure() {
		...
		RunChromosomeGetter.currentStatus.setText("Finishing current image search and shutting down.");
	}
	public void exit() {
